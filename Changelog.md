...
> [!IMPORTANT]
> 本版本将 Service 内核日志归档到应用数据目录的 `logs/v2.9.5/service/`，并修复内核连接中断后的日志流清理和重连。

### 🐞 修复问题

- **Service 日志持久化**：Rust 后台独立订阅内核日志并写入 `service/service_latest.log`，与应用日志和 Sidecar 日志统一存放在版本日志目录中；关闭窗口、进入轻量模式或在日志页暂停、清除、筛选时继续写盘。
- **日志文件轮转**：Service 日志沿用应用设置的文件大小上限和保留数量，应用日志级别不会过滤内核日志。
- **WebSocket 断线恢复**：TCP 与 Unix socket 在 EOF、关闭帧或传输错误后移除连接并释放回调，避免空循环阻止后台日志重连。
- **补录范围**：启动和重连后通过已认证的 IPC 补充服务保留的最近 100 条输出，可能与实时日志重叠；较长断线期间的记录不保证全部补齐。服务端受保护副本仍保留，已轮转历史日志不自动迁移。
...
- GitHub Actions `Release Build` 成功复现 DMG、上传公开资产并完成构建来源证明；最终公开产物已通过 `hdiutil verify`、镜像内应用的 `codesign --verify --deep --strict` 以及三个 Service Helper 的独立签名校验（74,044,760 bytes），SHA256：`6c497b65b663bff3fab3ea19898a3ccd8ae26110fe8396b361fc97c014edcf17`；已同步至 `target/aarch64-apple-darwin/release/bundle/dmg/Clash_Verge_2.9.5_aarch64.dmg` 与 `/Users/felix/Downloads/Clash_Verge_2.9.5_aarch64.dmg`。
